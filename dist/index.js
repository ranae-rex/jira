...
module.exports = {
	getIssueId: function (issueKey) {
		var options = {
			method: 'GET',
			uri: 'https://' + process.env.jiraOrganisation + '.atlassian.net/rest/api/2/issue/' + issueKey,
			headers: {
				'Authorization': 'Basic ' + Base64.btoa(process.env.jiraUsername + ':' + process.env.jiraPassword),
			},
		...
		return Promise.resolve(rp(options)
			...
			.catch(function (err) {
				console.log(`filaed to get issueId by key ${err}`);
				return err;
			}))
	},
